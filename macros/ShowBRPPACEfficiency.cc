...
void ShowBRPPACEfficiency::Construct() {
  ...
  h_br_zed_dummy  = new TH2F("ZedDummy","ZedDummy",12,0,12,100,20,40);
  h_br_zed_ppac_x = new TH2F("ZedGateX","ZedGateX",12,0,12,100,20,40); 
  h_br_zed_ppac_y = new TH2F("ZedGatey","ZedGateY",12,0,12,100,20,40); 

  h_br_ppac_eff_x = new TH2F("PPACEffX","PPACEffX",12,0,12,100,20,40); 
  h_br_ppac_eff_y = new TH2F("PPACEffY","PPACEffY",12,0,12,100,20,40); 
...
void ShowBRPPACEfficiency::Calculate()  {
  ...
  for(int i =0;i<12;i++){
    ...
    if(ppac){ 
       cout<<i<<", tx1 ="<<ppac->GetTX1Raw()<<",tx2 = "<<ppac->GetTX2Raw()<<endl;
