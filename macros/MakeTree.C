...
void MakeTree(char*, char*);
void MakeTree(char*, char*, int);
...
int main(int argc, char *argv[]){
  /*
  if( (!(argc == 2 && 4==strlen(argv[1]))) && (3!=argc)){
    std::cout<< "============================================== " <<std::endl;
    std::cout<< "run with "<<std::endl;
    std::cout<< "  ./MakeTree [input root file] [output root file] " <<std::endl;
    std::cout<< "or "  << std::endl;
    std::cout<< "  ./MakeTree [input run number with 4-digits]" <<std::endl;
  ...
  if( (!(argc == 2 && 4>= strlen(argv[1]) )) && (3!=argc)){
    ...
    std::cout<< "  ./MakeTree [input root file] [output root file] " <<std::endl;
    std::cout<< "or "  << std::endl;
    std::cout<< "  ./MakeTree [input run number (e.g., for run0038, both 38 and 0038 are OK.)]" <<std::endl;
    std::cout<< "stop.. "<<std::endl;
    return -1;
  }
  ...
  if(2==argc && 4>= strlen(argv[1])){
    ...
    sprintf(inputfilename,"ridf/%04d.ridf",runnumber);
    //    sprintf(outputfilename,"root_raw/raw%s.root",argv[1]);
    sprintf(outputfilename,"root_raw/raw%04d.root",runnumber);
    MakeTree(inputfilename, outputfilename, runnumber);
  }
    return 0;
}


void MakeTree(char *infile, char *outfile){
  return MakeTree(infile, outfile, -999);
}


void MakeTree(char *infile, char *outfile, int runnum){

  std::cout << "MakeTree: " << infile << " -> "<< outfile << std::endl;
...
//  tree->Branch("F11PLA2_TR_NumHit",&F11PLA2_TR_NumHit,"F11PLA2_TR_NumHit/I");
  ...
  if(1<=runnum && runnum <=20){
    platdc_cut_min_l[0]= 105000. +4500.;  platdc_cut_max_l[0]= 120000. +4500.;//F3PLA
    platdc_cut_min_r[0]= 105000. +4500.;  platdc_cut_max_r[0]= 120000. +4500.;//F3PLA
    //
    platdc_cut_min_l[1]= 105000. +4500.;  platdc_cut_max_l[1]= 120000. +4500.;//F5PLA
    platdc_cut_min_r[1]= 105000. +4500.;  platdc_cut_max_r[1]= 120000. +4500.;//F5PLA
    //
    platdc_cut_min_l[2]= 105000. +4500.;  platdc_cut_max_l[2]= 120000. +4500.;//F7PLA
    platdc_cut_min_r[2]= 105000. +4500.;  platdc_cut_max_r[2]= 120000. +4500.;//F7PLA
    //
    platdc_cut_min_l[3]= 105000. +4500.;  platdc_cut_max_l[3]= 120000. +4500.;//F8PLA
    platdc_cut_min_r[3]= 105000. +4500.;  platdc_cut_max_r[3]= 120000. +4500.;//F8PLA
    //
    platdc_cut_min_l[4]= 120000. +4500.;  platdc_cut_max_l[4]= 135000. +4500.;//F11PLA
    platdc_cut_min_r[4]= 120000. +4500.;  platdc_cut_max_r[4]= 135000. +4500.;//F11PLA
    //
    platdc_cut_min_l[5]= 120000. +4500.;  platdc_cut_max_l[5]= 135000. +4500.;//F11LONG
    platdc_cut_min_r[5]= 120000. +4500.;  platdc_cut_max_r[5]= 135000. +4500.;//F11LONG
    //
    platdc_cut_min_l[6]= 120000. +4500.;  platdc_cut_max_l[6]= 135000. +4500.;//F11VETO
    platdc_cut_min_r[6]= 120000. +4500.;  platdc_cut_max_r[6]= 135000. +4500.;//F11VETO
    
  }else if(21<=runnum && runnum <= 147){
...
//  tree->Branch("F11IC1NumHit",&F11IC1NumHit,"F11IC1NumHit/I");
  ...
  tree->Branch("F11IC1TDCraw_multi",F11IC1TDCraw_multi,Form("F11IC1TDCraw[8][%d]/D",max_multihit_f11tdc));
...
//  tree->Branch("F11IC2NumHit",&F11IC2NumHit,"F11IC2NumHit/I");
  ...
  tree->Branch("F11IC2TDCraw_multi",F11IC2TDCraw_multi,Form("F11IC2TDCraw[8][%d]/D",max_multihit_f11tdc));
    ...
    for(int i=0;i<rawevent -> GetNumSeg();i++){
      ...
      if(fpl==63 && detector==10){
        ...
        for(int j=0;j<seg->GetNumData();j++){
          ...
          if(0==strcmp(plapara->GetDetectorName()->Data(),"F3pl")){
            if(mm==*((TArtRIDFMap *)plapara->GetTLMap())){
              ...
              F3PLA_TL_NumHit ++;
              //   std::cout << "F3TL numhit=" <<F3PLA_TL_NumHit << std::endl;
        ...
        for(Int_t j=0;j<seg->GetNumData();j++){
          ...
          //search (i,j)
          ////std::cout << ppacpara->GetDetectorName()->Data() << std::endl;
          ...
          ////std::cout << "i,j= " << i_ppac_focalplane << " ,  " <<  j_ppac <<std::endl;
   ...
   //   F11VETO_T = tpla->GetTime();
    ...
    for(int ii=0; ii<7; ii++){
      for(int jj=0; jj<4; jj++){
        ...
        PPACX[ii][jj]   = tppac->GetX();
        PPACY[ii][jj]   = tppac->GetY();
    ...
    if(tic) {
      ...
      for(int ich=0; ich<6; ich++){
        F7ICADCraw[ich] = tic->GetRawADC(ich);
      }
    }
    ...
    if(tic) {
      ...
      for(int ich=0; ich<8; ich++){
        F11IC1ADCraw[ich] = tic->GetRawADC(ich);
      }
    }
    ...
    if(tic) {
      ...
      for(int ich=0; ich<8; ich++){
        F11IC2ADCraw[ich] = tic->GetRawADC(ich);
      }
    }
    //
    //------
    ...
    //F11TDC other channel, no need to init. all are copied from F11VMETDCraw, which were initialized.
    {
      ...
      ii = 31;//F11TDC-DAQtrigger reference
