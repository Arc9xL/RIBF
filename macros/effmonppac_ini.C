void effmonppac_ini(Int_t runnum){
	...
	estore->Open();
	//---------------------------------------------------------------------------------------------
	//estore->Open(Form("ridf/bigrips/run%04d.ridf",runnum));
	//---------------------------------------------------------------------------------------------
	//BigRIPS Parameters
	TArtBigRIPSParameters *setup = TArtBigRIPSParameters::Instance();
	setup->LoadParameter((char*)"db/BigRIPSPPAC.xml");//BigRIPS CAMAC
	//setup->LoadParameter((char*)"db/BigRIPSPPAC.xml_sva03_20141023");//BigRIPS CAMAC
...
*/
	...
	TLatex *e53 = new TLatex(0.20,0.56,Form("F11/1A/X = %1.5f",100*eff111ax/ntriggerbit)); e53 -> SetNDC(); e53 -> SetTextSize(0.03); e53 -> Draw();
	TLatex *e54 = new TLatex(0.20,0.53,Form("F11/1A/Y = %1.5f",100*eff111ay/ntriggerbit)); e54 -> SetNDC(); e54 -> SetTextSize(0.03); e54 -> Draw();
	TLatex *e55 = new TLatex(0.20,0.50,Form("F11/1B/X = %1.5f",100*eff111bx/ntriggerbit)); e55 -> SetNDC(); e55 -> SetTextSize(0.03); e55 -> Draw();
	TLatex *e56 = new TLatex(0.20,0.47,Form("F11/1B/Y = %1.5f",100*eff111by/ntriggerbit)); e56 -> SetNDC(); e56 -> SetTextSize(0.03); e56 -> Draw();
	TLatex *e57 = new TLatex(0.20,0.44,Form("F11/2A/X = %1.5f",100*eff112ax/ntriggerbit)); e57 -> SetNDC(); e57 -> SetTextSize(0.03); e57 -> Draw();
	TLatex *e58 = new TLatex(0.20,0.41,Form("F11/2A/Y = %1.5f",100*eff112ay/ntriggerbit)); e58 -> SetNDC(); e58 -> SetTextSize(0.03); e58 -> Draw();
	TLatex *e59 = new TLatex(0.20,0.38,Form("F11/2B/X = %1.5f",100*eff112bx/ntriggerbit)); e59 -> SetNDC(); e59 -> SetTextSize(0.03); e59 -> Draw();
	TLatex *e60 = new TLatex(0.20,0.35,Form("F11/2B/Y = %1.5f",100*eff112by/ntriggerbit)); e60 -> SetNDC(); e60 -> SetTextSize(0.03); e60 -> Draw();
